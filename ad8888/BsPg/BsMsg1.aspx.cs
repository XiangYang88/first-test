...
public partial class ad8888_BsPg_BsMsg1 : AdminPage
{
    private string TblName = "BS_Msg";
    ...
    private void initCtrl()
    {
        ...
        setViewState("CodeMT", Request.QueryString["CodeMT"]);
    ...
    private void bindData()
    {
        ...
        try
        {
            ...
            //string sql = "select a.*,b.Name as UserName from {0} a left join bs_user b on a.Bs_UserID=b.id where {1} order by {2}";
            string sql = "select * from {0} where {1} order by {2}";
            string codeMT = getViewState("CodeMT");
            codeMT = string.IsNullOrEmpty(codeMT) ? "1" : codeMT;
            paras += string.Format(" and Sy_CodeMT='{0}'", codeMT);
            sql = string.Format(sql, TblName, paras, "id desc");
