...
public class getRemoteImage : IHttpHandler
{
    ...
    {
        string direc=DateTime.Now.ToString("yyyy-MM-dd") + "/";
        string savePath = context.Server.MapPath("/uploadfiles/images/" + direc);       //保存文件地址
        ...
        try
        {
            for (int i = 0, len = imgUrls.Length; i < len; i++)
            {
                ...
                tmpNames.Add("/uploadfiles/images/" + direc + tmpName);
            }
        }
        catch (Exception)
        {
