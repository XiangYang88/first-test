<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ucHeaderMCN.ascx.cs" Inherits="uc_ucHeaderMCN" %>
<header>
    <div class="header-top">
        <a href="default.aspx" class="logo">
            <img src="/upload/<%=BLL.Sys.Config.getConfigVal("logo") %>" alt="<%=BLL.Sys.Config.getConfigVal("webname")%>">
        </a>
        <%--<a class="lang" href="/m/en/default.aspx"><i class="icon-globe"></i></a>--%>
...
<div class="catalog">
    <ul>
        ...
        <%if (dr["code"].ToString() == "0106")
          { %>
        <li><a href="honor.aspx?code=<%=dr["code"]%>"><%=dr["Name"]%></a></li>
        <%}
          else
          { %>
        <li><a href="about.aspx?code=<%=dr["code"]%>"><%=dr["Name"]%></a></li>
        <%} %>
        <%}
        ...
        <%if (dr["code"].ToString() == "0701")
          { %>
        <li><a href="support.aspx?code=<%=dr["code"]%>"><%=dr["Name"]%></a></li>
        <%}
          else if (dr["code"].ToString() == "0702")
          { %>
        <li><a href="download.aspx?code=<%=dr["code"]%>"><%=dr["Name"]%></a></li>
        <%} %>
        <%}
          else if (dr["code"].ToString().Substring(0, 2) == "02")
          { %>
        <li><a href="news.aspx?code=<%=dr["code"]%>"><%=dr["Name"]%></a></li>
        <%}
          else if (dr["code"].ToString().Substring(0, 2) == "03")
          { %>
        <%if (dr["code"].ToString() == "0301")
          { %>
        <li><a href="hr.aspx?code=0301"><%=dr["name"]%></a></li>
        <%}
          else if (dr["code"].ToString() == "0302")
          { %>
        <li><a href="job.aspx?code=0302"><%=dr["name"]%></a></li>
        <%} %>
        <%}
        ...
        <%if (dr["code"].ToString() == "0401")
          { %>
        <li><a href="contact.aspx?code=0401"><%=dr["name"]%></a></li>
        <%}
          else if (dr["code"].ToString() == "0402")
          { %>
        <li><a href="message.aspx?code=0402"><%=dr["name"]%></a></li>
        <%} %>
        <%} %>
        <%}
          } %>
        ...
        <%if (dtProkind.Rows.Count > 0)
          {
              ...
              { %>
        <li><a href="products.aspx?code=<%=drP["code"]%>"><%=drP["name"]%></a>
        </li>
        <%}
              } %>
        <%} %>
